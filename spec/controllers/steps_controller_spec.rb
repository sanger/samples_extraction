    ...
    context "POST /activities/:activity/step_types/:step_type/steps" do
      ...
      it "execute a rule with different relations in between" do
       ...
       # skip('this rule is unsupported as condition_group.compatible_with? does not support loops while following relations')
        ...
        post :create, { :activity_id => @activity.id, :step_type_id => @step_type.id, :step => { :data_params => "{}"}}, format: :json
        Step.all.reload
        assert_equal Step.all.count, c+1
        assert_equal false, Step.last.in_progress?
      ...
      it "create a new step with status 'in progress' when pairing parameters are provided" do
        skip 'not supported'
