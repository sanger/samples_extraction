...
RSpec.describe Parsers::CsvLayout::LocationParser do
  ...
  it 'can parse a line' do
    expect(Parsers::CsvLayout::LocationParser.new(["A01", "F123456"], main_parser).location).to eq("A01")
  end
  it 'can pad the location' do
    expect(Parsers::CsvLayout::LocationParser.new(["A1", "F123456"], main_parser).location).to eq("A01")
  end

  it 'validates when the location is right' do
    parser = Parsers::CsvLayout::LocationParser.new(["A1", "F123456"], main_parser)
    expect(parser).to be_valid
  end

  it 'does not validate when the location is wrong' do
    parser = Parsers::CsvLayout::LocationParser.new(["A111", "F123456"], main_parser)
